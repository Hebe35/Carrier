...
#define HOUSE_HEAT_DROP_RELAY_PIN  A11 // Pin for the house heat drop relay 
...
const long HouseHeatDropOn          = 16;   // House heat drop ON temp  16 C°
const long HouseHeatDropOff         = 14;   // House heat drop OFF temp 14 C°
...
// House heat drop state
int houseHeatDropState;
...
void setup()
{
 ...
 // Water stop relay
  ...
  pinMode(HOUSE_HEAT_DROP_RELAY_PIN , OUTPUT);

 // Default mode for the house heat drop relay is OFF
  digitalWrite(HOUSE_HEAT_DROP_RELAY_PIN , HIGH); // House heat drop relay to OFF state
...
// qv,puh = puhallusilman määrä [m3/h]
  ...
  if (client.connect("www.emoncms.org", 80)) {
    ...
    // houseHeatDropState
    client.print(",house_heat_drop_state:");
    if ( houseHeatDropState  == true) {
      client.print("0");
    } else {
      client.print("1");
    } 
...
void readMG811() {
  ...
  const float v400ppm = 1.11;   //MUST BE SET ACCORDING TO CALIBRATION -> FREE AIR 2.85  USE 1.11 16.05.2015
  const float v40000ppm = 0.40; //MUST BE SET ACCORDING TO CALIBRATION -> FREE AIR 1.87  USE 0.40 16.05.2015
...
void alarmHouseHeatDrop()   
{
 ...
 // if outside temp goes to a on > 16 - house heat drop on
 if (outdoorTemp  >= HouseHeatDropOn ) {// outdoor temp
   digitalWrite(HOUSE_HEAT_DROP_RELAY_PIN, LOW);
   houseHeatDropState = true;
   Serial.println("house heat drop state ON");
   
   // Alarm goes to a on state - house heat drop on
   } else if (outdoorTemp  >= -20 && alarmState == LOW) {// outdoor temp
    digitalWrite(HOUSE_HEAT_DROP_RELAY_PIN, alarmState);
     houseHeatDropState = true;
     Serial.println("house heat drop state ON");
     
  // if outside temp goes to a on < 14 - house heat drop off    
    } else if(outdoorTemp  <= HouseHeatDropOff ) {// outdoor temp{
    houseHeatDropState = false;
    digitalWrite(HOUSE_HEAT_DROP_RELAY_PIN, HIGH);
    Serial.println("house heat drop state OFF");
  }
...
void waterHeat() 
{
 ...
 // If water goes lever to a on state - water heat on
  if (boilerTopTemp  <= WaterBoilerTopHeatOn ) {// boiler_top temp
