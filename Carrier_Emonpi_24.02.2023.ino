...
// check if returns are valid, if they are NaN (not a number) then something went wrong!
  ...
  } else if (fireplace >=FireplaceFanOn && fireplace <= 100 && fireplace != 127) {
  ...
  } else if ( utility >= 23 && utility < 100 && utility != 127) { 
    // Default to MODE_FAN with FAN 1
    operatingMode = MODE_FAN;
    temperature = 22;
    fanSpeed = FAN_1;
  ...
  } else if ( kitchen >= 23 && kitchen < 100 && kitchen != 127) { 
    // Default to MODE_FAN with FAN 1
    operatingMode = MODE_FAN;
    temperature = 22;
    fanSpeed = FAN_1;
...
// qv,puh = puhallusilman määrä [m3/h]
  ...
  if (client.connect("192.168.1.144", 80)) {
    ...
    for (int i=0; i < sizeof(owbuses) / sizeof(struct owbus); i++) {
      if ((owbuses[i].temperature != DEVICE_DISCONNECTED) && (owbuses[i].temperature > -100) && (owbuses[i].temperature < 100) && (owbuses[i].temperature != 127)) { 
        if (notFirst) {
          client.print(",");
        }
        notFirst = true;
