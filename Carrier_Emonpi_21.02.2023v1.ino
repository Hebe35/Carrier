...
void requestTemperatures()
{
  ...
  timer.after(850, readTemperatures); // Vakio750 ..450-850
}
...
// check if returns are valid, if they are NaN (not a number) then something went wrong!
  ...
  } else if (fireplace >=FireplaceFanOn && fireplace <= 60) {
  ...
  // Set the mode based on the outdoor temperature (summer cooling)
   if (outdoor >= 20 &&  aircond_intake >= 23 && outdoor < 60 && aircond_intake < 60) {
    operatingMode = MODE_COOL;
    temperature = 25;
    fanSpeed = FAN_AUTO;

    
    if (outdoor >= 22 && outdoor <= 23 && aircond_intake >= 23 && outdoor < 60 && aircond_intake < 60) {
      temperature = 24;
    } else if (outdoor >= 24 && outdoor <= 25 && aircond_intake >= 23 && outdoor < 60 && aircond_intake < 60) {
      temperature = 23;  
    } else if (outdoor >= 26 && outdoor <= 27 && aircond_intake >= 23 && outdoor < 60 && aircond_intake < 60) {
      temperature = 22;  
    } else if (outdoor >= 28 && outdoor <= 29 && aircond_intake >= 23 && outdoor < 60 && aircond_intake < 60) {
      temperature = 21;    
    } else if (outdoor >= 30 && aircond_intake >= 23 && outdoor < 60 && aircond_intake < 60) {
      temperature = 20;    
    }
  // Fireplace is hot, use the FAN mode
  } else if (fireplace >= 25 && fireplace < 60) {
  ...
  // Utility room is hot, as the laundry drier has been running
    ...
    } else if (utility >= 27 && outdoor >= 20 && utility < 60 && outdoor < 60) {
  ...
  // Kitchen is hot, as the oven has been running
    ...
    } else if (kitchen >= 27 && outdoor >= 20 && kitchen < 60 && outdoor < 60) {
      // COOL with AUTO FAN, +24
      operatingMode = MODE_COOL;
      temperature = 24;
      fanSpeed = FAN_AUTO;
...
// Log the water meter pulses
 ...
 // check if returns are valid, if they are NaN (not a number) then something went wrong!
    ...
    if ((!isnan(DHT11Humidity)) ||  DHT11Humidity < -1 ||  DHT11Humidity > 101){ // || DHT11Humidity == NaN) {
     client.print(DHT11Humidity);
     } else {
      client.print(",");
     }
    client.print(",dht11_temperature:");
    if (!isnan((DHT11Temperature)) ||  DHT11Temperature < -60 || DHT11Temperature > 60) {
...
void readDHT11() {
  ...
  // check if returns are valid, if they are NaN (not a number) then something went wrong!
   if ((!isnan(DHT11Humidity))|| DHT11Humidity < -1 || DHT11Humidity > 101) {
...
void alarmHouseHeatDrop()   
{
 ...
 // Log sensor temperatures
      ...
      if (owbuses[3].temperature != DEVICE_DISCONNECTED) { // || owbuses[3].temperature < -60 || owbuses[3].temperature > 60) {
          JuliaBedroomTemp = owbuses[3].temperature;
         } else {
        JuliaBedroomTemp = JuliaBedroomTemp;
         }
      if (owbuses[4].temperature != DEVICE_DISCONNECTED) { // || owbuses[4].temperature < -60 || owbuses[4].temperature > 60) {
          masterBedroomTemp = owbuses[4].temperature;
         } else {
        masterBedroomTemp = masterBedroomTemp;
        }
      if (owbuses[5].temperature != DEVICE_DISCONNECTED) { // || owbuses[5].temperature < -60 || owbuses[5].temperature > 60) {
 ...
 if (outdoor  >= HouseHeatOff && outdoor < 60) {// outdoor
   ...
   } else if (outdoor  >= -10 && outdoor < 60 && alarmState == LOW) {// outdoor
    digitalWrite(HOUSE_HEAT_RELAY_PIN, alarmState);
     houseHeatState = false;
     Serial.println("house heat state OFF");

   // if house heat Fission state goes to a off  - house heat OFF   
   } else if(HouseHeatFissioState == HIGH && JuliaBedroomTemp >= 18  && warehouseTemp >= 7 && masterBedroomTemp >= 18 && JuliaBedroomTemp < 60 && masterBedroomTemp && warehouseTemp < 60 ){
    houseHeatState = false;
    digitalWrite(HOUSE_HEAT_RELAY_PIN, LOW);
    Serial.println("house heat state OFF");

  // if Fissio heat goes to a on  - house heat on
   } else if (HouseHeatFissioState == LOW && outdoor <= HouseHeatOn || JuliaBedroomTemp  <= 19 || warehouseTemp <= 8 || masterBedroomTemp <= 19 && JuliaBedroomTemp < 60 && masterBedroomTemp && warehouseTemp < 60 ) {
...
//  Serial.println("house heat state2 ON");
 ...
 if (housePower >= 20) {// housePower >= 20 kw
   ...
   // Alarm goes to a on state - house heat OFF
   } else if (outdoor  >= 15 && outdoor  <= 60 && alarmState == LOW) {// outdoor
    digitalWrite(HOUSE_HEAT_RELAY2_PIN, alarmState);
     houseHeatState2  = false;
     Serial.println("house heat state2 OFF");
     
  // if Fissio heat2 goes to a OFF  - house heat OFF  
    } else if(HouseHeatFissioState2  == HIGH && utility >= 19 && utility < 60){// { && outdoor  <= HouseHeatOn
    houseHeatState2 = false;
    digitalWrite(HOUSE_HEAT_RELAY2_PIN, LOW);
    Serial.println("house heat state2 OFF");

  // if Fissio heat2 goes to a on  - house heat on
   } else if (HouseHeatFissioState2  == LOW ||  utility <= 18) {
   digitalWrite(HOUSE_HEAT_RELAY2_PIN, HIGH);
   houseHeatState2 = true;
   Serial.println("house heat state2 ON");
...
// Log sensor temperatures
      ...
      if (owbuses[11].temperature != DEVICE_DISCONNECTED) { // || owbuses[11].temperature < -1 || owbuses[11].temperature > 100) {
 ...
 // If water goes lever to a on state - water heat on
  if (WaterHeatFissioState == LOW || boilerTopTemp <= WaterBoilerTopHeatOn && boilerTopTemp < 60 ) {
