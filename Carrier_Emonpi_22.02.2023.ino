...
int humidity_LTO = 0; // DHT11Humidity;
...
void readTemperatures()
{
  for (int i=0; i < sizeof(owbuses) / sizeof(struct owbus); i++) {
    ...
    if ((temperature != DEVICE_DISCONNECTED)  && temperature > -100 && temperature < 100 && temperature != 127) {
...
void updateDisplay()
{
  ...
  if ((( displayedSensor < sizeof(owbuses) / sizeof(struct owbus) &&
       owbuses[displayedSensor].temperature != DEVICE_DISCONNECTED)) && owbuses[displayedSensor].temperature != 127 && owbuses[displayedSensor].temperature > -100 && owbuses[displayedSensor].temperature < 100) {
...
// check if returns are valid, if they are NaN (not a number) then something went wrong!
    ...
    if ((!isnan(owbuses[7].temperature)) && owbuses[7].temperature > -100 && owbuses[7].temperature < 100 && owbuses[7].temperature != 127) {
      aircond_intake = owbuses[7].temperature;
    }
    if ((!isnan(owbuses[14].temperature)) && owbuses[14].temperature > -100 && owbuses[14].temperature < 100 && owbuses[14].temperature != 127) {
       outdoor = owbuses[14].temperature;
    }
    if ((!isnan(owbuses[0].temperature)) && owbuses[0].temperature > -100 && owbuses[0].temperature < 100 && owbuses[0].temperature != 127) {
       fireplace = owbuses[0].temperature;
    }
    if ((!isnan(owbuses[2].temperature)) && owbuses[2].temperature > -100 && owbuses[2].temperature < 100 && owbuses[2].temperature != 127) {
       utility = owbuses[2].temperature;
    }
     if ((!isnan(owbuses[1].temperature)) && owbuses[1].temperature > -100 && owbuses[1].temperature < 100 && owbuses[1].temperature != 127) {
       kitchen = owbuses[1].temperature;
    }
     if ((!isnan(DHT11Humidity)) && DHT11Humidity > -100 && DHT11Humidity < 100 && DHT11Humidity != 127) {
      humidity_LTO = DHT11Humidity;
    }
  ...
  } else if (fireplace >=FireplaceFanOn && fireplace <= 60 && fireplace != 127) {
  ...
  // Set the mode based on the outdoor temperature (summer cooling)
   if (outdoor >= 20 &&  aircond_intake >= 23 && outdoor < 60 && aircond_intake < 60 && outdoor != 127 && aircond_intake != 127) {
    operatingMode = MODE_COOL;
    temperature = 25;
    fanSpeed = FAN_AUTO;

    
    if (outdoor >= 22 && outdoor <= 23 && aircond_intake >= 23 && outdoor < 60 && aircond_intake < 60 && outdoor != 127 && aircond_intake != 127) {
      temperature = 24;
    } else if (outdoor >= 24 && outdoor <= 25 && aircond_intake >= 23 && outdoor < 60 && aircond_intake < 60 && outdoor != 127 && aircond_intake != 127) {
      temperature = 23;  
    } else if (outdoor >= 26 && outdoor <= 27 && aircond_intake >= 23 && outdoor < 60 && aircond_intake < 60 && outdoor != 127 && aircond_intake != 127) {
      temperature = 22;  
    } else if (outdoor >= 28 && outdoor <= 29 && aircond_intake >= 23 && outdoor < 60 && aircond_intake < 60 && outdoor != 127 && aircond_intake != 127) {
      temperature = 21;    
    } else if (outdoor >= 30 && aircond_intake >= 23 && outdoor < 60 && aircond_intake < 60 && outdoor != 127 && aircond_intake != 127) {
      temperature = 20;    
    }
  // Fireplace is hot, use the FAN mode
  } else if (fireplace >= 25 && fireplace < 60 && fireplace !=  127) {
    ...
    if (fireplace >= 26 && fireplace < 28) {
      ...
      fanSpeed = FAN_4;
    } else if ( fireplace >= 33 && fireplace < 60) {
      fanSpeed = FAN_5;
    }
  // Utility room is hot, as the laundry drier has been running
  } else if ( utility >= 23 && utility < 60 && utility != 127) { 
    // Default to MODE_FAN with FAN 1
    operatingMode = MODE_FAN;
    temperature = 22;
    fanSpeed = FAN_1;
    ...
    } else if (utility >= 27 && outdoor >= 20 && utility < 60 && outdoor < 60 && outdoor != 127) {
  ...
  // Kitchen is hot, as the oven has been running
  } else if ( kitchen >= 23 && kitchen < 60 && kitchen != 127) { 
    // Default to MODE_FAN with FAN 1
    operatingMode = MODE_FAN;
    temperature = 22;
    fanSpeed = FAN_1;
    ...
    } else if (kitchen >= 27 && outdoor >= 20 && kitchen < 60 && outdoor < 60 && kitchen != 127 && outdoor != 127) {
      // COOL with AUTO FAN, +24
      operatingMode = MODE_COOL;
      temperature = 24;
      fanSpeed = FAN_AUTO;
  ...
  } else if (humidity_LTO < 50 ) {
    // FAN with FAN 1 temp+22
    operatingMode = MODE_HEAT;
    temperature = 20;
    fanSpeed = FAN_AUTO;

    if ((outdoor >= 15 && outdoor < 20) && humidity_LTO < humidityILPNotHeat ) {
      temperature = 21;
    } else if ((outdoor >= 5 && outdoor < 15) && humidity_LTO < humidityILPNotHeat ) {
      temperature = 22;
    } else if ((outdoor >= 0 && outdoor < 5) && humidity_LTO < humidityILPNotHeat ){ // -1~500w 0~750w 1~1150w  2~1500w
      temperature = 23;
    } else if ((outdoor >= -4 && outdoor < 0) && humidity_LTO < humidityILPNotHeat ) {
      temperature = 24;
    } else if ((outdoor >= -9 && outdoor < -4) && humidity_LTO < humidityILPNotHeat ) {
      temperature = 25;
    } else if ((outdoor >= -14 && outdoor < -9) && humidity_LTO < humidityILPNotHeat ) {
      temperature = 26;
    } else if ((outdoor >= -19 && outdoor < -14) && humidity_LTO < humidityILPNotHeat ) {
      temperature = 27;
    } else if ((outdoor >= -24 && outdoor < -19) && humidity_LTO < humidityILPNotHeat ) {
      temperature = 28;
    } else if ((outdoor >= -29 && outdoor < -24) && humidity_LTO < humidityILPNotHeat ) {
      temperature = 29;
    } else if ((outdoor >= -35 && outdoor < -29) && humidity_LTO < humidityILPNotHeat ) {
      temperature = 30;
    } else if ((outdoor >= -45 && outdoor < -35) && humidity_LTO < humidityILPNotHeat ) {
      temperature = 30;
    }
 ...
 if (heatpumpAirFlowRate  < 100 && humidity_LTO < humidityILPNotHeat ){
 ...
 if (heatCOPEEROff != 0 && humidity_LTO < humidityILPNotHeat ){
    operatingMode = carrierHeatpump.operatingMode;
    fanSpeed = carrierHeatpump.fanSpeed;
    temperature = carrierHeatpump.temperature;
     // ILP heat cable control
...
// qv,puh = puhallusilman määrä [m3/h]
  ...
  if (client.connect("192.168.1.144", 80)) {
    ...
    for (int i=0; i < sizeof(owbuses) / sizeof(struct owbus); i++) {
      if ((owbuses[i].temperature != DEVICE_DISCONNECTED) && owbuses[displayedSensor].temperature != 127 && owbuses[displayedSensor].temperature > -100 && owbuses[displayedSensor].temperature < 100) { 
        if (notFirst) {
          client.print(",");
        }
        notFirst = true;
...
// Log the water meter pulses
 ...
 // check if returns are valid, if they are NaN (not a number) then something went wrong!
    ...
    if (!isnan((DHT11Temperature)) && DHT11Temperature > -60 && DHT11Temperature < 60 && DHT11Temperature != 127) {
    ...
    // if (!isnan(LTOefficient)) { // || LTOefficient < -100 || LTOefficient > 120) {
    //  client.print(" ");
    // } else {
      client.print(LTOefficient);  
    // }
    ...
    // if (!isnan(LTOefficientIn)) { // && LTOefficientIn < -100 || LTOefficientIn > 120) {
    //  client.print("1");
    // } else {
      client.print(LTOefficientIn);  
    // }  
    ...
    // if (!isnan(LTOefficientOut)) { // || LTOefficientOut > -100 || LTOefficientOut > 120) {
    //  client.print("1");
    // } else {
      client.print(LTOefficientOut);  
    // }
...
void readDHT11() {
  ...
  // check if returns are valid, if they are NaN (not a number) then something went wrong!
   if ((!isnan(DHT11Humidity)) && DHT11Humidity > -1 && DHT11Humidity < 101 && DHT11Humidity != 127) {
    DHT11Humidity = humidityDht;
    humidity_LTO = humidityDht;
   } else {
    DHT11Humidity = DHT11Humidity;
    humidity_LTO = humidity_LTO;
     }
   if ((!isnan(DHT11Temperature)) && DHT11Temperature > -60 && DHT11Temperature < 60 && DHT11Temperature != 127) {    
...
void alarmHouseHeatDrop()   
{
 ...
 // Log sensor temperatures
      ...
      if ((owbuses[3].temperature != DEVICE_DISCONNECTED) && owbuses[3].temperature > -60 && owbuses[3].temperature < 60 && owbuses[5].temperature != 127) {
          JuliaBedroomTemp = owbuses[3].temperature;
         } else {
        JuliaBedroomTemp = JuliaBedroomTemp;
         }
      if ((owbuses[4].temperature != DEVICE_DISCONNECTED) && owbuses[4].temperature > -60 && owbuses[4].temperature < 60  &&owbuses[4].temperature != 127) {
          masterBedroomTemp = owbuses[4].temperature;
         } else {
        masterBedroomTemp = masterBedroomTemp;
        }
      if ((owbuses[5].temperature != DEVICE_DISCONNECTED) && owbuses[5].temperature > -60 && owbuses[5].temperature < 60 && owbuses[5].temperature != 127) {
 ...
 if (outdoor  >= HouseHeatOff && outdoor < 60) {// outdoor
   ...
   } else if (outdoor  >= -10 && outdoor < 60 && alarmState == LOW && outdoor != 127) {// outdoor
    digitalWrite(HOUSE_HEAT_RELAY_PIN, alarmState);
     houseHeatState = false;
     Serial.println("house heat state OFF");

   // if house heat Fission state goes to a off  - house heat OFF   
   } else if(HouseHeatFissioState == HIGH && JuliaBedroomTemp >= 18  && warehouseTemp >= 7 && masterBedroomTemp >= 18 && JuliaBedroomTemp < 60 && masterBedroomTemp && warehouseTemp < 60 && JuliaBedroomTemp != 127 && masterBedroomTemp != 127 && warehouseTemp != 127){
    houseHeatState = false;
    digitalWrite(HOUSE_HEAT_RELAY_PIN, LOW);
    Serial.println("house heat state OFF");

  // if Fissio heat goes to a on  - house heat on
   } else if (HouseHeatFissioState == LOW && outdoor <= HouseHeatOn || JuliaBedroomTemp  <= 19 || warehouseTemp <= 8 || masterBedroomTemp <= 19 && JuliaBedroomTemp < 60 && masterBedroomTemp && warehouseTemp < 60 && JuliaBedroomTemp != 127 && masterBedroomTemp != 127 && warehouseTemp != 127 ) {
...
//  Serial.println("house heat state2 ON");
 ...
 if (housePower >= 20) {// housePower >= 20 kw
   ...
   // Alarm goes to a on state - house heat OFF
   } else if (outdoor  >= 15 && outdoor  <= 60 && alarmState == LOW && outdoor != 127) {// outdoor
    digitalWrite(HOUSE_HEAT_RELAY2_PIN, alarmState);
     houseHeatState2  = false;
     Serial.println("house heat state2 OFF");
     
  // if Fissio heat2 goes to a OFF  - house heat OFF  
    } else if(HouseHeatFissioState2  == HIGH && utility >= 19 && utility < 60 && utility != 127) {// { && outdoor  <= HouseHeatOn
    houseHeatState2 = false;
    digitalWrite(HOUSE_HEAT_RELAY2_PIN, LOW);
    Serial.println("house heat state2 OFF");

  // if Fissio heat2 goes to a on  - house heat on
   } else if (HouseHeatFissioState2  == LOW ||  utility <= 18 && utility != 127) {
   digitalWrite(HOUSE_HEAT_RELAY2_PIN, HIGH);
   houseHeatState2 = true;
   Serial.println("house heat state2 ON");
...
// Log sensor temperatures
      ...
      if ((owbuses[11].temperature != DEVICE_DISCONNECTED) && owbuses[11].temperature > -1 && owbuses[11].temperature < 100 && owbuses[11].temperature != 127) {
          boilerTopTemp = owbuses[11].temperature;
         } else {
          boilerTopTemp = boilerTopTemp;
         } 
         
 // If water goes lever to a on state - water heat on
  if (WaterHeatFissioState == LOW || boilerTopTemp <= WaterBoilerTopHeatOn && boilerTopTemp < 60 && boilerTopTemp != 127 ) {
